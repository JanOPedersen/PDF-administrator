...
class ModelMySQL(object):
    ...
    def delete(self, id):
        backend.mySQLdelete(self._connection, id)

    def update(self, id, flags):
        backend.mySQLupdate(self._connection, id, flags)

    def getSelected(self, filename):
        return backend.mySQLgetSelected(self._connection, filename)

    def getFromId(self, id):
        return backend.getFromId(self._connection, id)

    def getFromKeywords(self, keywords, flags, retAll):
        return backend.getFromKeywords(self._connection, keywords, flags, retAll)

    def getFromFilepath(self, filepath, flags, retAll):
        return backend.getFromFilepath(self._connection, filepath, flags, retAll)

    def getFromFlags(self, flags):
        return backend.getFromFlags(self._connection, flags)

    def updateNumbers(self):
        backend.updateNumbers(self._connection)
...
class ViewController(object):
    ...
    def __init__(self, model):
        ...
        ButtonCommands = [self.insert, self.delete, self.update, self.get, 
                          self.getSelected, self.showSelected, self.clearAll]

        btnX = listBoxX
        for i in range(len(ButtonTxts)):
            btn = Button(self.root, text=ButtonTxts[i], font=('italic', 10), 
                         bg='white', command = ButtonCommands[i])
    ...
    def get(self):
        try:
            ...
            if (self.e_id.get() == "" and self.e_filename.get() == "" 
                and self.e_filepath.get() == "" and self.retAll.get() == 0):
